...
class Plan_model extends CI_Model {
	...
	function get_month_plan_no_insp($month_year, $filters) {
		 $sql = "SELECT mp.id, mp.`month_year`, mp.`supplier_id`, mp.`product_id`, mp.`part_id`,tr.`start_date`,
        mp.`planned_part_no`, mp.`test_id`, mp.schedule_date,mp.no_inspection,
        p.name as product, s.name as supplier, pp.name as part,
        t.name as test,
        CASE 
        ...
        $sql .= " GROUP BY mp.`month_year`, mp.product_id,mp.`supplier_id`, mp.part_id ORDER BY pp.name ,mp.planned_part_no, s.name ";
       // $sql .= " GROUP BY mp.product_id,  mp.`supplier_id`,mp.part_id
	...
	function get_month_plan_mail($month_year, $filters) {
		// print_r($filters);exit;
        $sql = "select * from (SELECT mp.id, mp.`month_year`, mp.`supplier_id`, mp.`product_id`, mp.`part_id`, 
        mp.`planned_part_no`, mp.`test_id`, mp.schedule_date,mp.no_inspection,
        p.name as product, s.name as supplier, pp.name as part,
        t.name as test,
        CASE 
